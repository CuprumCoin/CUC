...
module System_dependencies = struct
  ...
  let precheck ?(using_docker = false) ?(protocol_paths = [])
    ...
    | more, `Or_fail -> (
        ...
        ( if
          List.exists more ~f:(function
            | `Missing_exec ("cuprum-node", _)
              when Caml.Sys.file_exists ("." // "cuprum-node") ->
                true
            | _ -> false)
        then
          Console.say state
            EF.(
              desc (prompt "Tip:")
                (wf
                   "The `Cuprum-node` executable is missing but there seems to \
                    be one in the current directory, maybe you can pass \
                    `./Cuprum-node` with the right option (see `--help`) or \
  ...
  let build state ~clients =
    let aliases =
      List.concat_mapi clients ~f:(fun i c ->
          ...
          let extra =
            let help = "Call the cuprum-client used by the sandbox." in
          ...
          [ ( sprintf "tc%d" i
            , cmd "cuprum-client"
            , "Call the `cuprum-client` from the path." ) ]
          @ extra) in
    make ~aliases
